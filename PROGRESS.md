...
- **Registro de Decisões Arquitetônicas (ADR)**: Criado o diretório `.sdd/decisions/` para documentar decisões estruturais, modais e correções do projeto de forma categorizada. As primeiras decisões documentadas cobrem a evolução de dropdowns para seleção múltipla (`arquitetura.md`), resolução de aninhamento de listas (`warnings_e_correcoes.md`) e controle de transições de abertura de modais (`modais.md`).
